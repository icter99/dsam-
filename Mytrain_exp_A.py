...
from segment_anything.modeling.depth_prompt_head import DepthGeometryPromptHead
...
log_dir = join('log', 'exp_A')
result_dir = join('train_result', 'exp_A')
os.makedirs(log_dir, exist_ok=True)
os.makedirs(result_dir, exist_ok=True)

timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
log_file = join(log_dir, f'expA_train_{timestamp}.log')
result_csv = join(result_dir, f'expA_metrics_{timestamp}.csv')
...
# Attach DGPH module dynamically
sam_model.dgph = DepthGeometryPromptHead(256, 256).to(device)

sam_model.train()
# Set up the optimizer, hyperparameter tuning will improve performance here
# Only optimize mask_decoder and the new DGPH head
optimizer = torch.optim.Adam(
    list(sam_model.mask_decoder.parameters()) + list(sam_model.dgph.parameters()),
...
for epoch in range(1, num_epochs+1):
    ...
    for step, (image_embedding, img, gt2D, boxes, boundary, depth_embedding) in enumerate(tqdm(train_dataloader, desc=f'Epoch {epoch}')):
        ...
        geo_prompt = sam_model.dgph(depth_embedding)
