...
def validate(model, val_dataset, device):
    ...
    with torch.no_grad():
        for image_embedding, img, gt2D, boxes, boundary, depth_embedding in val_loader:
            ...
            final_mask = model.loop_finer(mask_predictions, image_embedding, depth_embedding)
...
log_dir = join('log', 'exp_B')
result_dir = join('train_result', 'exp_B')
os.makedirs(log_dir, exist_ok=True)
os.makedirs(result_dir, exist_ok=True)

timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
log_file = join(log_dir, f'expB_train_{timestamp}.log')
result_csv = join(result_dir, f'expB_metrics_{timestamp}.csv')
...
# Fine-tune mask_decoder and loop_finer together
